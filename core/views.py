...
from django.views.decorators.http import require_POST
import numpy as np
from .models import Medical, Profile, Ment
import joblib
...
def MakeMend(request):
    try:
        ...
        # Fetching patient's age, gender, weight, and severity from Profile model
        patient_id = Medical.objects.filter(pk=disease).values_list('patient_id', flat=True).first()
        patient_profile = Profile.objects.get(user_id=patient_id)

        age = patient_profile.get_age()
        sex = patient_profile.get_gender_numeric()
        weight = patient_profile.convert_weight_to_numeric()
        severity = patient_profile.convert_severity_to_numeric()   
       
        
        
        # Mapping disease names to unique identifiers
        disease_dict = {'Acne': 0, 'Allergy': 1, 'Diabetes': 2, 'Fungal infection': 3, 'Urinary tract infection': 4,
                        'Malaria': 5, 'Migraine': 6, 'Hepatitis B': 7, 'AIDS': 8}
        ...
        medical_instance = Medical.objects.get(pk=disease)
        disease_name = medical_instance.disease
        dis=disease_name
        print(dis)
        # Checking if the diagnosed disease is in the dictionary
        if dis in disease_dict:
            print('AI Got Drug For This Disease')
            new_sick = disease_dict.get(dis)
            
            # Preparing input for the model
            test = [new_sick, sex, age, weight, severity]
            print(test)
            test = np.array(test).reshape(1, -1)
            print(test.shape)
          
            # Loading the model and making prediction
            clf = joblib.load('model/medical_dt.pkl')
            prediction = clf.predict(test)
            prediction = prediction[0]
            print('Predicted Drug Is',prediction)

            # Updating the Medical model with the predicted disease
...
def SaveMent(request):
    try:
        ...
        print(f"pk: {pk}, day: {day}, time: {time}")

        # Check if the Ment object exists
        check_ment = Ment.objects.filter(pk=pk).exists()

        if check_ment:
            Ment.objects.filter(id=pk).update(approved=True, ment_day=day, time=time, doctor_id=request.user.id)
